...
class Post(models.Model):
    ...
    #TextField:글자수제한이없는 저장공간을 설정하는 클래스
    #null : 데이터베이스에 해당변수값이 없어도 저장되도록 설정하는 매개변수
    #blank : 사용자가 입력양식에 해당변수값을 빈칸으로 둬도 폼을 서버에 전달할수있도록
    # 설정하는매개변수. null과 blank 매개변수는 모든 Field클래스에서 사용할 수 있음
    content = models.TextField(null=True,blank=True)
    #auto_now_add(DateTimeField나 DateField에서 사용가능)
    #: 객체 생성 시, 서버 기준의 날짜/시간이 자동으로 입력되도록 설정하는 매개변수
...
class PostImage(models.Model):
    ...
    #단, 해당 필드를 사용할려면 Pillow 모듈이 파이썬에 설치가 되있어야함
