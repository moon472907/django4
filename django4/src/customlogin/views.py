...
def signup(request):
    ...
    if request.method == "POST":
        ...
        #True를 반환한 경우 : 사용자 입력이 유효한 값인 것을 확인. 
        ...
        if form.is_valid():
            ...
            if form.cleaned_data['password']==form.cleaned_data['password_check']:
                ...
                비밀번호를 저장해야되기 때문에, save()함수를 사용할 수 없음.
                User 모델클래스에 있는 회원생성 함수를 사용해 새로운 회원을 데이터베이스에 만들수
                있음. User.objects.create_user(아이디,비밀번호,이메일) : 매개변수에 들어온 입력을
...
def signin(request):
    ...
    #POST 방식 요청
        ...
        #username과 비밀번호가 일치하는 User 객체 한개를 추출하는 함수
        ...
        if u:
            ...
            #정보로 로그인 됨. 로그인 된 클라이언트는 request.user 변수를 사용해
