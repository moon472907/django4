...
단, app_name와 urlpatterns 변수를 정의해야함(이름이 들리면 URL Conf가 설정되지않음)
app_name : 이 파일에 정의한 URL들의 그룹이름(문자열)
urlpatterns : path함수를 이용해 뷰함수를 등록하는 변수 (리스트)
만들어진 urls.py를 프로젝트폴더의 urls.py에 알려줘야함
'''
...
urlpatterns=[
    ...
    #127.0.0.1:8000/vote/qr/ -> qregiste 함수 호출 
    #파이썬코드나 HTML에서 vote:qr 별칭으로 URL을 찾을 수 있음
    path('qr/', qregiste, name='qr'),
    #127.0.0.1:8000/vote/cr/ -> cregiste 함수 호출
    #파이썬코드나 HTML에서 vote:cr 별칭으로 URL을 찾을 수 있음
