...
비로그인 상태인 웹클라이언트에게 로그인페이지를 리다이렉트 해주는 데코레이터
...
def qregiste(request):
    ...
    if request.method == "GET":
        ...
        return render(request,'vote/qregiste.html',
                      {'form' : obj.as_table()})
    #POST 방식요청
...
def qupdate(request, q_id):
    ...
    if request.method =='GET':
        ...
        #HTML 파일 전달
        return render(request,'vote/qupdate.html',
                      {'form':obj.as_p()})
    #POST 방식 요청
    elif request.method == "POST":
...
def qdelete(request, q_id):
    ...
    print('삭제후id :', q.id)
    return render(request,'vote/delete_com.html',
                  {'title': q.name, 'type': 1 })
#Choice 객체 추가
@login_required
def cregiste(request):
    ...
    if request.method == "GET":
        ...
        obj = ChoiceForm()
        return render(request, 'vote/cregiste.html', 
                      {'form':obj.as_table()})
    #POST 요청
    elif request.method == "POST":
        ...
        #->값이 비어있는 변수가 없기때문(votes는 기본값 설정이되있음)
        c = obj.save() #c:Choice 객체
        #다른 페이지로 이동(index나 detail로 이동)
        #return HttpResponseRedirect('/vote/') #index페이지 URL
        #연동된Question객체의 detail페이지의 URL
...
def cupdate(request, c_id):
    ...
    if request.method == "GET":
        ...
        return render(request,'vote/cupdate.html',
                      {'obj':obj})
    elif request.method == "POST":
